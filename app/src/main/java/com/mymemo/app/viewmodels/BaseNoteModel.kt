...
import com.mymemo.app.ActionMode
import com.mymemo.app.AttachmentDeleteService
import com.mymemo.app.Cache
import com.mymemo.app.Progress
import com.mymemo.app.R
import com.mymemo.app.ReminderReceiver
import com.mymemo.app.legacy.Migrations
import com.mymemo.app.legacy.XMLUtils
import com.mymemo.app.miscellaneous.Export
import com.mymemo.app.miscellaneous.IO
import com.mymemo.app.miscellaneous.Operations
import com.mymemo.app.miscellaneous.applySpans
import com.mymemo.app.preferences.AutoBackup
import com.mymemo.app.preferences.ListInfo
import com.mymemo.app.preferences.Preferences
import com.mymemo.app.preferences.SeekbarInfo
import com.mymemo.app.room.Attachment
import com.mymemo.app.room.Audio
import com.mymemo.app.room.BaseNote
import com.mymemo.app.room.Color
import com.mymemo.app.room.Converters
...
import com.mymemo.app.room.livedata.Content
import com.mymemo.app.room.livedata.SearchResult
...
class BaseNoteModel(private val app: Application) : AndroidViewModel(app) {
    ...
    private val notesObserver = androidx.lifecycle.Observer<List<com.mymemo.app.room.BaseNote>> {
        list ->
        Cache.list = list
    }
            ...
            withContext(Dispatchers.IO) {
                ...
                if (hasErrors) {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(app, "备份完成，但部分文件可能未成功备份", Toast.LENGTH_LONG).show()
                    }
                }
            }
    ...
    private fun importZipBackup(uri: Uri) {
        ...
        viewModelScope.launch(exceptionHandler) {
            ...
            withContext(Dispatchers.IO) {
                ...
                val databaseEntry = zipFile.getEntry(MyMemoDatabase.DatabaseName)

                val databaseFile = File(backupDir, MyMemoDatabase.DatabaseName)
    ...
    companion object {
        ...
        fun transform(list: List<BaseNote>, pinned: Header, others: Header): List<Item> {
            ...
            var addedOthersHeader = false
            for (baseNote in list) {
                if (!baseNote.pinned && !addedOthersHeader) {
                    newList.add(others)
