...
import com.mymemo.app.image.Event
...
import com.mymemo.app.preferences.BetterLiveData
...
import com.mymemo.app.room.SpanRepresentation
...
class MyMemoModel(private val app: Application) : AndroidViewModel(app) {
    ...
    fun setReminder(reminder: Reminder) {
        viewModelScope.launch {
            ...
            this@MyMemoModel.reminder.value = reminder
            updateReminder()
        }
    }
    ...
    suspend fun setState(id: Long) {
        withContext(Dispatchers.IO) {
            if (id != 0L) {
                ...
                val baseNote = cachedNote ?: baseNoteDao.get(id)

                if (baseNote != null) {
                    this@MyMemoModel.id = id
                    folder = baseNote.folder

                    color.postValue(baseNote.color)
                    labels.postValue(ArrayList(baseNote.labels))
                    images.postValue(baseNote.images)
                    audios.postValue(baseNote.audios)
                    reminder.postValue(baseNote.reminder)
    ...
    suspend fun saveNote(): Long {
        ...
        return withContext(Dispatchers.IO) { baseNoteDao.insert(baseNote) }
    }
